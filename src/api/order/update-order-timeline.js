...
module.exports = exports = {
  handler: async (req, res) => {
    const { id } = req.params;
    const { timelineId, isChecked } = req.body;
    const { user} = req;
    try {
      ...
      if (checkById) {
        let updatedTimeline = await global.models.GLOBAL.ORDER.updateOne(
          {
            _id: id,
            timeline: { $elemMatch: { _id: timelineId } },
          },
          {
            $set: {
              "timeline.$.date": Date.now(),
              "timeline.$.isChecked": isChecked,
            },
          }
        );
