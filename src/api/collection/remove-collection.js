    ...
    try {
      for (const i of collectionId) {
        const checkById = await global.models.GLOBAL.COLLECTION.findOne({
          ...
          path: "collectionMedia",
          model: "media",
        });
        if (checkById) {
          console.log("checkById", checkById);
          // ==================================== Delete from AWS and MEDIA TABLE ===========================================
          ...
          if (url) {
            ...
            let removeCategory = await global.models.GLOBAL.COLLECTION.findByIdAndDelete({
              _id: checkById._id,
            });
          }

          // ========================================= DELETE COLLECTION ONLY ===============================================
          //   let removeCollection = await global.models.GLOBAL.COLLECTION.findByIdAndDelete({
          //     _id: checkById._id,
          //   });
